...
Una vez los pacientes inicien sesión, se visualizará en la página principal los médicos que se han registrado en la plataforma exeptuando a los médicos con el cual el paciente ya tiene programado una cita.

![Dashboard Pacientes](./img/Dashboard_Paciente.png)

### 2.1 Registro y Acceso
*   **Pasos para registrarse:** Se debera llenar el formulario para los datos de registro del paciente, el registro sera válido si y solo si el paciente suba una `foto` y suba su `DPI` en un archivo de formato `PDF`. Una vez se llene todo el formulario y se presione el botón de registrar, debera esperar hasta que el adminsitrador apruebe su registro, de lo contrario no podrá ingresar con las credenciales de `correo electronico` y `password` que registro el paciene en el formulario de registro. **Cuando el administrador acepte el registro, se le enviará un correo de verifiación donde se le envía el token de verificación que necesitará para iniciar sesión en la plataforma**
   
![Formulario de Registro](./img/Registro%20Pacientes.png)

*   **Inicio de sesión:** Para poder ingresar a la plataforma como paciente, el administrador debe de aprobar su registro, de lo contrario nunca podrá ingresar con las credenciales ingresadas en el formulario de registro. Adicional al inicio de sesión se le pedirá que ingrese el código de verificación que se le manda a su correo registrado en el registro, si no ingresa dicho token de verificación, nunca podrá ingresar a la plataforma de `SaludPlus`.

![Token](./img/Token_Paciente.png)

![Login Token](./img/Verificación_Pacientes.png)
...
![Busqueda de Médicos](./img/Buscar_Médico_Paciente.png)
...
![Programar Cita para Pacientes](./img/ProgramarCitas_Pacientes.png)
...
![Visualizar Citas para Pacientes](./img/Citas_Pacientes.png)
...
![Historial Citas para Pacientes](./img/Hitorial_Pacientes.png)
...
![Cancelar Cita para Pacientes](./img/Cancelar_Cita_Paciente.jpeg)
...
![Perfil Pacientes](./img/Perfil_Pacientes.png)
...
![Prototipo_Adminstrador](./img/Prototipo_Medic_Paci_Marvelapp.png)

* **Link:** [Prototipo_Pacientes_Actualizado](https://marvelapp.com/prototype/75fgcdh)
...
![Dashboard_Médico](./img/Dashboard_Medico.png)

### 3.1 Registro y Acceso
*   **Pasos para registrarse:** Se debera llenar el formulario para los datos de registro del medico. En este rol todos los datos que se solicitan son de caracter obligatorio eso incluye la carga de la foto y sobre todo el `cv` del médico en formato `PDF`. Una vez se llene todo el formulario y se presione el botón de registrar, debera esperar hasta que el adminsitrador apruebe su registro, de lo contrario no podrá ingresar con las credenciales de `correo electronico` y `password` que registro el médico en el formulario de registro. **Cuando el administrador acepte el registro, se le enviará un correo de verifiación donde se le envía el token de verificación que necesitará para iniciar sesión en la plataforma**
   
![Formulario de Registro Médico](./img/Registro%20Medicos.png)

* **Login Médico:** Una vez que el adminsitrador válide la sólicitud se podrán ingresar las credenciales para iniciar sesión como médico. No olvide copiar o anotar el ``token`` de verificación para el inicio de sesión.

![Login Médico](./img/Token_Medico.png)

![Login Médico](./img/Verificación_Medico.png)
...
![Establer Horirio Médicos](./img/Citas_Medicos.png)
...
![Atender Pacientes Médico](./img/Atender_Paciente.png)
...
![Modificar Horario Médico](./img/Actualizar_Horario%20de%20Atención_Medicos.png)
...
![Historial de Citas Médico](./img/Historial_Médico.png)
...
![Perfil Médico](./img/Perfil_Médicos.png)
...
![Cancelar Citas Pacientes](./img/Cancelar%20Cita_Meico.jpeg)
...
![Correo de Cancelación](./img/CancelarCita_Medicos.png)
...
* **Link:** [Prototipo_Médicos_Actializado](https://marvelapp.com/prototype/75fgcdh)
...
![Login Administrador](./img/Login_Admin.png)
...
![Aceptar Solicitures M/P](./img/Aceptar%20Usuarios_Adminsitrador.png)

![Aceptar Solicitudes M/P](./img/Aceptar%20Users_Administrador.png)
...
![Aceptar Solicitudes M/P](./img/Dashboard_Admin.png)
...
![Aceptar Solicitudes M/P](./img/Dar%20De%20Baja_Adminstrador.png)
...
* **Doughnut Chast:** Esta gráfica reportará el horario con mayor demanda y la de menor demanda. Este reporte muestra de forma visual la preferencia de los usuarios `Pacientes` que agendan una cita con uno o varios médicos.

* **Gráfica de Barras:** Esta gráfica reportará las citas canceladas por especialidad del médico, modela la cantidad de citas canceladas que posee una especialidad, esto ayuda a visualizar la cantidad de citas que se cancelan ya sea por reportes de mala conducta del medico con la dicha especialización.

![Reportes_Administrador](./img/Reportes_Admin.png)
...
![Prototipo_Adminstrador](./img/Prototipo_Administrador_Marvelapp.png)

* **Link:** [Prototipo_Adminsitrador_Actualizado](https://marvelapp.com/prototype/35c270f6)
